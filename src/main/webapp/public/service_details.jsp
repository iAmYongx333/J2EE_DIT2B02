...
<head>
    ...
    <%
        ...
        String userRole = "";
        if (session.getAttribute("sessRole") != null) {
            userRole = session.getAttribute("sessRole").toString();
        }

        Service s = (Service) session.getAttribute("service");
        if (s == null) {
            response.sendRedirect(request.getContextPath() + "/public/services.jsp?errCode=ServiceNotFound");
            return;
        }

        ArrayList<Feedback> feedbackList =
            (ArrayList<Feedback>) session.getAttribute("feedbackList");
            ...
            <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
                ...
                <c:if test="${not empty feedbackList}">
                    <p class="text-[11px] text-slate-500">
                        ${fn:length(feedbackList)} review(s)
                    </p>
                </c:if>
            </div>
                            ...
                            <div class="feedback-card rounded-2xl border border-slate-200 bg-white px-4 py-4 sm:px-5 sm:py-5">
                                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                                    <div class="space-y-1.5">
                                        ...
                                        <p class="text-[11px] text-slate-500">
                                            Rating:
                                            <span class="font-semibold text-slate-900">${f.rating}</span>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white px-5 py-5 sm:px-6 sm:py-6">
                <form action="<%= request.getContextPath() %>/serviceDetailServlet" method="post" class="space-y-4">
                    ...
                    <div class="grid sm:grid-cols-[0.9fr_1.1fr] gap-4">
                        ...
                        <div class="space-y-1">
                            <label class="block text-sm font-medium text-slate-800">
                                Comments on the service / caregiver
                            </label>
